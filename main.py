...
# Flet 0.86.5 تابع app_storage_path ندارد.
# از پوشه Home برنامه استفاده می‌کنیم که در Android
# داخل فضای اختصاصی برنامه قرار می‌گیرد.
APP_DIR = Path.home() / ".ims_mobile"
...
def main(page: ft.Page):
    ...
    unit = ft.Dropdown(
        ...
        value="تکی",
        options=[
            ft.dropdown.Option("تکی"),
            ft.dropdown.Option("کارتن"),
        ],
    )
    ...
    movement_type = ft.Dropdown(
        ...
        value="ورود",
        options=[
            ft.dropdown.Option("ورود"),
            ft.dropdown.Option("خروج"),
        ],
    )
    ...
    async def open_scanner(_=None):
        try:
            perm = await ph.request(
                fph.Permission.CAMERA
            )

            if (
                perm is not None
                and perm != fph.PermissionStatus.GRANTED
            ):
                status.value = (
            ...
            back = next(
                (
                    c
                    for c in cameras
                    if c.lens_direction.value == "back"
                ),
                cameras[0],
            )
    ...
    async def capture_and_decode(_=None):
        if decode_barcodes is None:
            scanner_status.value = (
                "ماژول بارکدخوان در این محیط نصب نیست؛ "
                "بارکد را دستی وارد کنید."
            )
    ...
    def find_product(_=None):
        ...
        try:
            r = api_get(
                f"/api/v1/products/barcode/{code}"
            )
            ...
            if r.ok and data.get("success"):
                ...
                product_info.value = (
                    f"{p.get('name', '')} | موجودی: "
                    f"تکی {p.get('single_stock', 0)} / "
                    f"کارتن {p.get('carton_stock', 0)}"
                )
    ...
    def add_movement(_=None):
        ...
        if not p:
            status.value = (
                "ابتدا کالا را با بارکد پیدا کنید."
            )
        ...
        try:
            ...
            status.value = (
                ...
                "عملیات در گوشی ذخیره شد "
                "و بعداً ارسال می‌شود."
            )
    ...
    dialog = ft.AlertDialog(
        ...
        actions=[
            ft.TextButton(
                "خواندن بارکد",
                on_click=capture_and_decode,
            ),
            ft.TextButton(
                "بستن",
                on_click=close_scanner,
            ),
        ],
    )
